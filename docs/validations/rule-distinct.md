...
- Source file: `src/Horse.Validator.Rule.Distinct.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.Distinct().SetMessage('Distinct validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the Distinct rule."
    ]
  }
}
```
