...
- Source file: `src/Horse.Validator.Rule.uFile.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.File().SetMessage('File validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the File rule."
    ]
  }
}
```
