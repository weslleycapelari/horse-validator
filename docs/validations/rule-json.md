...
- Source file: `src/Horse.Validator.Rule.JSON.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.JSON().SetMessage('JSON validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the JSON rule."
    ]
  }
}
```
