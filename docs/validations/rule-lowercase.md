...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.Lowercase().SetMessage('Lowercase validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the Lowercase rule."
    ]
  }
}
```
