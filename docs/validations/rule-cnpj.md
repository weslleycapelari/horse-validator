...
- Source file: `src/Horse.Validator.Rule.CNPJ.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.CNPJ().SetMessage('CNPJ validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the CNPJ rule."
    ]
  }
}
```
