...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.ActiveUrl().SetMessage('ActiveUrl validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the ActiveUrl rule."
    ]
  }
}
```
