...
- Validates opt-in style accepted values: yes, on, 1, or true.
- Category: `Boolean`
...
- Source file: `src/Horse.Validator.Rule.Accepted.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.Accepted().SetMessage('Accepted validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the Accepted rule."
    ]
  }
}
```
