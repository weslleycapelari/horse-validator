...
- Source file: `src/Horse.Validator.Rule.URL.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.URL().SetMessage('URL validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the URL rule."
    ]
  }
}
```
