...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.IpAddress().SetMessage('IpAddress validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the IpAddress rule."
    ]
  }
}
```
