...
- Source file: `src/Horse.Validator.Rule.ULID.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.ULID().SetMessage('ULID validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the ULID rule."
    ]
  }
}
```
