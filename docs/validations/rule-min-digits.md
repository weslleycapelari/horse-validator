...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.MinDigits(3).SetMessage('MinDigits validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the MinDigits rule."
    ]
  }
}
```
