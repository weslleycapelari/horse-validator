...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.MACAddress().SetMessage('MACAddress validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the MACAddress rule."
    ]
  }
}
```
