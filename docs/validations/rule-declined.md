...
- Validates opt-out style declined values: no, off, 0, or false.
- Category: `Boolean`
...
- Source file: `src/Horse.Validator.Rule.Declined.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.Declined().SetMessage('Declined validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the Declined rule."
    ]
  }
}
```
