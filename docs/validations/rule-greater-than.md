...
- Source file: `src/Horse.Validator.Rule.GreaterThan.pas`
...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.GreaterThan(10.5))
  .Validate;
```
...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.GreaterThan('referenceField'))
  .Validate;
```
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.GreaterThan(10.5).SetMessage('GreaterThan validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the GreaterThan rule."
    ]
  }
}
```
