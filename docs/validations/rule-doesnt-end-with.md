...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.DoesntEndWith(['.exe', '.bat']))
  .Validate;
```
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.DoesntEndWith(['.exe', '.bat']).SetMessage('DoesntEndWith validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the DoesntEndWith rule."
    ]
  }
}
```
