...
- Source file: `src/Horse.Validator.Rule.IsArray.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.IsArray().SetMessage('IsArray validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the IsArray rule."
    ]
  }
}
```
