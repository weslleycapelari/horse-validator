...
- Category: `Array`
- Category behavior: Targets collection constraints such as uniqueness and item count bounds.
...
- Source file: `src/Horse.Validator.Rule.MinCount.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.MinCount(3).SetMessage('MinCount validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the MinCount rule."
    ]
  }
}
```
