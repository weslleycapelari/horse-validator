...
- Source file: `src/Horse.Validator.Rule.UUID.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.UUID().SetMessage('UUID validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the UUID rule."
    ]
  }
}
```
