...
- Category behavior: Checks JSON structure type assumptions (object/array).
...
- Source file: `src/Horse.Validator.Rule.IsObject.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.IsObject().SetMessage('IsObject validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the IsObject rule."
    ]
  }
}
```
