...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.AlphaDash().SetMessage('AlphaDash validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the AlphaDash rule."
    ]
  }
}
```
