...
- Source file: `src/Horse.Validator.Rule.EndsWith.pas`
...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.EndsWith(['.com', '.org']))
  .Validate;
```
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.EndsWith(['.com', '.org']).SetMessage('EndsWith validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the EndsWith rule."
    ]
  }
}
```
