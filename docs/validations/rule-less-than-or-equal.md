...
- Source file: `src/Horse.Validator.Rule.LessThanOrEqual.pas`
...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.LessThanOrEqual(10.5))
  .Validate;
```
...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.LessThanOrEqual('referenceField'))
  .Validate;
```
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.LessThanOrEqual(10.5).SetMessage('LessThanOrEqual validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the LessThanOrEqual rule."
    ]
  }
}
```
