...
- Source file: `src/Horse.Validator.Rule.MaxCount.pas`
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.MaxCount(3).SetMessage('MaxCount validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the MaxCount rule."
    ]
  }
}
```
