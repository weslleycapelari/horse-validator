...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.MimeTypes(['image/png', 'image/jpeg']))
  .Validate;
```
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.MimeTypes(['image/png', 'image/jpeg']).SetMessage('MimeTypes validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the MimeTypes rule."
    ]
  }
}
```
