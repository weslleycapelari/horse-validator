...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.DoesntStartWith(['admin_', 'sys_']))
  .Validate;
```
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.DoesntStartWith(['admin_', 'sys_']).SetMessage('DoesntStartWith validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the DoesntStartWith rule."
    ]
  }
}
```
