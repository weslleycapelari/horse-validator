...
- Validates date/time after or equal to a literal date or referenced field.
- Category: `Date`
...
- Source file: `src/Horse.Validator.Rule.AfterOrEqual.pas`
...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.AfterOrEqual(EncodeDate(2026, 3, 16)))
  .Validate;
```
...
THorseValidator.New(Req)
  .Field('body.value')
  .AddRule(TRules.AfterOrEqual('referenceField'))
  .Validate;
```
...
THorseValidator.New(Req)
  ...
  .Field('body.value').AddRule(TRules.AfterOrEqual(EncodeDate(2026, 3, 16)).SetMessage('AfterOrEqual validation failed for this field.'))
  .Validate;
```
...
```json
{
  ...
  "validations": {
    "body.value": [
      "The field body.value failed the AfterOrEqual rule."
    ]
  }
}
```
