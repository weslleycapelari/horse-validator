...
type
  /// <summary>
  ///   Valida se a data do campo é estritamente anterior a uma data de referência.
  /// </summary>
  /// <remarks>
  ///   Suporta comparação com uma data fixa (Literal) ou com a data contida em outro campo (Referência).
  /// </remarks>
  TRuleBefore = class(TRuleContextAware)
  strict private
    FRefDate: TArgument<TDateTime>;
  public
    /// <summary>
    ///   Cria uma nova instância da regra 'Before'.
    /// </summary>
    /// <param name="ARefDate">A data (ou referência) que serve como limite superior exclusivo.</param>
...
function TRuleBefore.Validate(const AFieldName, AValue: string; const ARequired: Boolean): Boolean;
var
  LValue: string;
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      SetResultMessage(Format('Erro ao obter a data de referência para validação Before: %s', [E.Message]));
      Exit(False);
    end;
  end;
  ...
  begin
    if LIsReference then
      SetResultMessage(Format('O campo %s deve ser uma data anterior ao campo %s.', [AFieldName, FRefDate.RefName]))
    else
      SetResultMessage(Format('O campo %s deve ser uma data anterior a %s.', [AFieldName, DateTimeToStr(LTargetDate)]));
  end;
end;
